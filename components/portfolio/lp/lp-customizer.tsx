...
import { lpColorPresets, type LpTemplate } from "./templates";

type DisplayMode = "pc" | "sp";

const viewportSpecs: Record<
  DisplayMode,
  { label: string; detail: string; width: number; height: number; scale: number }
> = {
  pc: {
    label: "ノートPC",
    detail: "1440 × 900",
...
export function LpCustomizer({ template }: { template: LpTemplate }) {
  const [primary, setPrimary] = useState("#2563EB");
  const [button, setButton] = useState("#111827");
  const [background, setBackground] = useState("#ffffff");
  const [radius, setRadius] = useState(18);
  const [fontSize, setFontSize] = useState(24);
  const [mode, setMode] = useState<DisplayMode>("pc");
  const [headline, setHeadline] = useState("テキストテキスト");
      ...
      <div className="min-w-0">
        ...
        <div className="overflow-auto pb-2">
          <div
            ...
            style={{ width: frameWidth, height: frameHeight, padding: bezel }}
          >
            {mode === "pc" ? (
      ...
      <Card className="h-fit lg:sticky lg:top-6">
        ...
        <CardContent className="space-y-6 pt-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <ColorField label="背景カラー" value={background} onChange={setBackground} />
          </div>

          <RangeField label="角丸サイズ" value={radius} min={4} max={30} suffix="px" onChange={setRadius} />
          <RangeField label="フォントサイズ" value={fontSize} min={18} max={34} suffix="px" onChange={setFontSize} />

          <div className="space-y-2">
            <p className="text-xs font-medium text-black/52">表示モード</p>
            <div className="grid grid-cols-2 gap-2">
              <Button variant={mode === "pc" ? "default" : "outline"} onClick={() => setMode("pc")}>
                <Monitor size={15} />
                PC
              </Button>
              <Button variant={mode === "sp" ? "default" : "outline"} onClick={() => setMode("sp")}>
                <Smartphone size={15} />
                SP
              </Button>
            </div>
